...
const UserProfile = () => {
    ...
    const handleInputChange = (e) => {
        ...
        if (["facebook", "instagram", "youtube", "twiiter"].includes(name)) {
            let platform = name === "twiiter" ? "twitter" : name;
            if (!isValidUrl(value, platform)) {
                setErrors((prev) => ({
                    ...prev,
                    [name]: `Please enter a valid ${platform} URL.`,
                }));
            } else {
                setErrors((prev) => ({
                    ...prev,
    ...
    const validateSocialLinks = () => {
        const platforms = [
            ...
            { name: "twiiter", label: "Twitter" }
        ];
        let isValid = true;
        let newErrors = {};

        platforms.forEach(({ name, label }) => {
            let platform = name === "twiiter" ? "twitter" : name;
            if (formData[name] && !isValidUrl(formData[name], platform)) {
                isValid = false;
                newErrors[name] = `Please enter a valid ${label} URL.`;
            } else {
                newErrors[name] = "";
            }
        });
    ...
    const handleUpdateProfile = (e) => {
        ...
        if (!validateSocialLinks()) {
            toast.error("Please enter valid social media URLs.");
            return;
        }

        // Check if profile image is not available
        if (!formData.profileImage) {
            toast.error(translate("pleaseAddProfileImage"));
            return;
        }

    ...
    >
      <div className="card verticle_main_card">
        <div className="verticle_card_img_div">
          ...
          />
          <div className="premium_icon_and_feature_div">
            {ele?.is_premium ? (
            ...
            {ele?.promoted ? (
              <span className="feature_tag">{translate("featured")}</span>
            ) : null}
          </div>
          ...
          <div className="feature_card_mainbody">
            {ele?.category && (
              <div className="cate_image">
                {themeEnabled ? (
                  <ImageToSvg
                    imageUrl={ele?.category && ele?.category.image}
                    className="custom-svg"
                  />
                ) : (
                  <Image
                    loading="lazy"
                    src={ele?.category && ele?.category.image}
                    alt="no_img"
                    width={20}
                    height={20}
                    onError={placeholderImage}
          ...
          <div className="category_params">
            <div className="row">
              {validParameters.map((elem, index) => (
                <Tooltip
                  title={
                    Array.isArray(elem?.name)
                      ? elem.name.slice(0, 1).join(", ")
                      : elem?.name
                  }
                  placement="top"
                >
                  <div
                    className="col-6 col-sm-6 col-lg-3 ant-tooltip-open"
                    key={index}
                  >
                    <div className="cat_content">
                      <div className="param_img">
                        {themeEnabled ? (
                          <ImageToSvg
                            imageUrl={elem?.image && elem?.image}
                            className="custom-svg"
                          />
                        ) : (
                          <Image
                        ...
                        <span>
                          <span>
                            {Array.isArray(elem?.value)
                              ? truncate(elem.value.slice(0, 1).join(", "), 5)
        ...
        <div
          className={`card-footer ${
            validParameters.length > 0 ? "auto_hight" : "custom_hight"
          }`}
          id="feature_card_footer"
        >
          {ele.property_type && (
            <span
              className={`${
                ele?.property_type === "sell" ? "for_sale" : "for_rent"
              } `}
            >
              {" "}
              {""}
          ...
          {ele?.price && (
            <span className="price_tag">
              <span>{formatPriceAbbreviated(ele?.price)}</span>
            </span>
          )}
        </div>
      </div>
